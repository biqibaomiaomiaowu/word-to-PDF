...
### 关于 PDF 转 Word 的双路线自动分流架构
本项目针对 PDF 转 Word 采用了“自动分流双路线方案”，旨在针对不同复杂度的 PDF 采取最合适的解析策略，而非让所有 PDF 强行走同一条链路：

1. **自动预分析与路由 (`pdf_route_selector`)**
   - 转换前使用轻量化 `PyMuPDF` 提取文本块密度、图片数量、绘图对象数量以及特殊数学符号等特征，进行快速启发式检测。
   - 根据检测结果，系统会自动决定该文档适合走哪条路线。
   - 在此阶段，系统还会检测并**预先剥离高置信度的尾页广告**，从源头防止广告页被恢复进最终的 DOCX 中。

2. **标准文本引擎 (`pdf2docx` 路线)**
   - **适用场景**: 文字层完整、表格不复杂、图片不多、版面较规则的标准 PDF。
   - **特点**: 速度快，文本流恢复较好。

3. **复杂版面引擎 (`PaddleOCR / PP-Structure` 路线)**
   - **适用场景**: 复杂版面、图片型、数学教辅类 PDF（包含较多公式、密集表格或复杂图表）。
   - **特点**: 重点利用版面分析 (Layout Analysis)、表格识别 (Table Recognition) 等技术将复杂结构恢复到 Word。不再单纯依赖后处理硬救。
   - *注：当前本地 POC 默认使用 CPU 模型运行。*

4. **有限降级与回退机制**
   - 如果复杂 PDF 在 Paddle 引擎下转换完全失败，或者转换后经系统质量校验（段落、表格、排版等维度）得分极差，系统允许执行**一次有限回退**至 `pdf2docx` 引擎进行尝试，并对比得分取较优者，同时在结果中明确标注。不会无底线地把极差结果伪装成成功。
