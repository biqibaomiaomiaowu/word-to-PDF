...
def find_default_pdf(repo_root: Path) -> Path | None:
    preferred = sorted(repo_root.glob("专题10.3*.pdf"))
    if preferred:
        return preferred[0]

    all_pdfs = sorted(repo_root.glob("*.pdf"))
    if all_pdfs:
        return all_pdfs[0]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run PPStructureV3 conversion and print output stats.")
    parser.add_argument("--input", help="Input PDF path. Defaults to the sample PDF in repo root.")
    parser.add_argument("--device", choices=["auto", "cpu", "gpu:0"], default="auto", help="Execution device")
    parser.add_argument(
        "--output-dir",
        default="tmp_structure_test",
